...
def addAllSightlines(ax, fig):
    ...
    _otherBand = np.array([2,12,22,32,42,52,62, 3,13,23,33,43,53,63,  4,14,24,34,44,54,64, 5,15,25,35,45,55,65])
    
    index = np.argwhere(_all==1);_all = np.delete(_all, index)
    index = np.argwhere(_all==9);_all = np.delete(_all, index)
    
    chordsToPlot = _otherBand
    ...
    tangentAngles = 180*np.arccos(L_matrix['maxParallelity'])/np.pi
    print(f'tangentAngles: {tangentAngles}')   
    maxVal = np.max(tangentAngles)*1.05#maximum number of counts seen by any 1 detector. Used for the colorbar
    ...
    for i in range(len(chords)):
        addParticularSightline(ax, chords[i], tangentAngles[i], L_tgs[i], R_tgs[i], Z_tgs[i], maxVal, relevantEtendues[i])


    ###### Set up colorbar ######
    cmap = matplotlib.cm.ScalarMappable(norm = matplotlib.colors.Normalize(0,maxVal),
    ...
    cbar.set_label(r"Max parallelity")
    plt.rc('font', **{'size':'10'})
    #############################
