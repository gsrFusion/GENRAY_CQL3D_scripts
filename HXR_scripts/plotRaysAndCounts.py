...
detectorArea_cm2 = constants.detectorRadius_m**2*1e4
...
def addAllSightlines(ax, fig):
    ...
    BuildLmatrix.main(chordsToPlot,.8, saveFilename = f'lengthMatrices/Lmat{shotNum}_plotRaysAndCounts')
    L_matrix = np.load(f'lengthMatrices/Lmat{shotNum}_plotRaysAndCounts.npz')
    ...
    _,countMatrix = CountMatrix.getCountMatrix(chords, attenuate = True, includeResponseFunc = False, E_pMin= 50, E_pMax = 250)
    countMatrix = countMatrix[0] + countMatrix[1]
    detectorCounts = np.sum(countMatrix, axis = -1)
    
    maxCounts = np.max(detectorCounts)#maximum number of counts seen by any 1 detector. Used for the colorbar

    etendues = DetectorInformation.getEtendues()
    relevantEtendues = etendues[chords-1]

    #add the sightlines one by one
    for i in range(len(chords)):
        addParticularSightline(ax, chords[i], detectorCounts[i], L_tgs[i], R_tgs[i], Z_tgs[i], maxCounts, relevantEtendues[i])


    ###### Set up colorbar ######
    scaleFactor = 1e4
    cmap = matplotlib.cm.ScalarMappable(norm = matplotlib.colors.Normalize(0,maxCounts/scaleFactor),
         cmap = plt.get_cmap('viridis'))
    cmap.set_array([])
    ticks = np.linspace(0,maxCounts/scaleFactor,5)

    formatter = tkr.ScalarFormatter(useMathText=True)
    formatter.set_powerlimits((0,0))

    cbar = fig.colorbar(cmap, ax = ax, shrink = .7, ticks = ticks, format = formatter)
    cbar.set_label(r"counts per second ($10^{4}$/second)")
    plt.rc('font', **{'size':'10'})
    #############################
...
"""This code should probably be reviewed. Seems like an area where an error is likely"""
def addParticularSightline(ax, chordNum, detectorCount, L_tg, R_tg, Z_tg, maxCounts, etendue):
    coneHalfAngle = np.arcsin(np.sqrt(etendue/(np.pi*detectorArea_cm2)))
    spotSizeRadius = np.tan(coneHalfAngle)*L_tg

    color = cm.viridis(detectorCount/maxCounts)

    ax.add_patch(Circle((R_tg, Z_tg), facecolor = color, edgecolor = color, zorder = 3, radius = spotSizeRadius))
    textColor = 'w'
    if detectorCount > .85*maxCounts:
...
def drawFluxSurfaces(ax):
    ...
    ax.contour(rInterp, zInterp, psirzNormInterp, np.square(np.arange(0,1,.1)), colors= 'k', linewidths= 1.5, linestyle = 'dashed', zorder = 4)
