...
def plotLos():
    fig, axes = plt.subplots(2, dpi = 100)
    fig.suptitle("Poloidal and Toroidal Lines of Sight")
    #fig.set_size_inches(6, 12)
    
    for chordNum in chordsToPlot:
        leftAngle, rightAngle, _, __ = DetectorInformation.getDivergenceAngles(chordNum)
        addLos(axes, x_sxr,y_sxr,thet2[chordNum-1], thet1[chordNum-1], leftAngle, rightAngle)

    setupToroidalLos(fig, axes[1])
    setupPoloidalLos(fig, axes[0])

    fig.tight_layout(rect=[0, 0.0, 1, 0.97])
    plt.show()


def setupPoloidalLos(fig, ax):
    ax.set_xlabel("R (m)")
    ax.set_ylabel("z (m)")
...
def drawFluxSurfaces(ax):
    ...
    #np.arange(0,1.1,.1)
    #ax.contour(r, z, psirzNorm, np.square(np.arange(0,1.1,.1)), colors= 'k')
...
def addLos(axs, xDec, yDec, torTheta, polarTheta, leftAngle, rightAngle):
    ...
    axs[0].plot(np.sqrt(centerXs**2 + centerYs**2), centerZs, color = 'g')
    axs[1].fill_between(leftXs_rot, leftYs_rot, rightYs_rot, alpha=.5, facecolor='b', edgecolor='None')
