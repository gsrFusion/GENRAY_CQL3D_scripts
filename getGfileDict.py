...
#if no target directory path is passed in, use target in getTargetInfo
#assumes the target is of the form /X/X/X/X/MACHINE_shots/MACHINE_SHOTNUM/targetDirectory
def getGfileDict(targetDir = None):
    ...
    print(f"shot: {shot}, machine: {machine}")  
    #look up the name of the gfile so we can open it
    eqdskName = shotToEqdsk.getEqdskName(shot, machine = machine)
    
    fileLoc = f'{targetDir}/{eqdskName}'
