...
def plotSPAMatrix():
    ...
    for i in range(len(NPara_fors)):
        ...
        if NPara_fors[i] > 0:
            prefix = 'p'
        targetDir = f'{stem}{prefix}{np.abs(NPara_fors[i])}Npara_300kW_3modules_1nScale_1Tscale'
        print(f'targetDir: {targetDir}')

        cql_nc = netCDF4.Dataset(f'{targetDir}/cql3d.nc','r')
        curr = cql_nc.variables["curr"][-1,:]*1e4/1e3#convert to kA/m^2
        
        J[i] = np.max(curr)#totalCD
    ...
    ax2 = ax.twinx()
    ax2.plot(NPara_fors, J,lw=2, color = 'tab:orange')
    ax2.yaxis.label.set_color('tab:orange')
    ax2.set_ylabel(r'J$_{LH}$ (kA/m$^2$)')       
